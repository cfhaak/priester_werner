...
document.addEventListener("DOMContentLoaded", () => {
    ...
    loadSnippetMetadata()
        .then((metadata) => {
            filepathsByLabels = metadata;
            populateColumns();
            addButton("column-adder", "Add Column", addNewColumn);
            addButton("scroll-toggler", "Toggle Scrolling", toggleScrollingBehaviour);
            addButton("empty-line-toggler", "Toggle Empty Line Visibility", toggleEmptyLinesVisibility);
            addButton("global-linenr-toggler", "Toggle Global Line Counter", toggleGlobalLinecounterVisibility);
            addButton("local-linenr-toggler", "Toggle Individual Line Counter", toggleLocalLinecounterVisibility);
        })
...
async function loadSnippetMetadata() {
    try {
        ...
        console.error(`Error loading snippet metadata from ${snippetLogPath}:`, error);
        return {};
    }
}

function getSnippet(uid, callback) {
    if (snippetsByLabels[uid]) {
        callback(snippetsByLabels[uid]);
    } else {
        fetchSnippet(filepathsByLabels[uid])
            .then((snippet) => {
                snippetsByLabels[uid] = snippet;
                callback(snippet);
            })
            .catch((error) => {
...
function generateDropdown(columnId, defaultValue) {
    return `
        <select class="text-select" onchange="updateColumn('${columnId}', null, this)">
            ${Object.keys(filepathsByLabels)
                .sort()
                .map(label => `<option value="${label}" ${label === defaultValue ? "selected" : ""}>${label}</option>`)
                .join("")}
        </select>`;
}
...
function createColumnHTML(columnId, defaultText) {
    const cssClass = global_scroll === true ? GLOBAL_SCROLL_CLASS : INDIVIDUAL_SCROLL_CLASS;
    return `
        <div id="${columnId}" class="witness ${cssClass}">
            <div class="controls-container">
                ${generateDropdown(columnId, defaultText)}
                <button class="remove-column-button" onclick="removeColumn('${columnId}')" title="Remove Column">&times;</button>
            </div>
            <div class="text-content ${cssClass}">${defaultText || "Error while loading."}</div>
        </div>`;
}

function populateColumns(defaultColumnNumber = 5) {
    witnessContainer.innerHTML = "";
    const labelKeys = Object.keys(filepathsByLabels).sort();
    if (!labelKeys.length) {
    ...
    for (let i = 1; i <= defaultColumnNumber; i++) {
        columnCount++;
        const currentKey = labelKeys[i - 1];
        if (currentKey) createColumn(witnessContainer, i, currentKey);
    }
}

function createColumn(container, columnIndex, key) {
    const columnId = `witness_column_${String(columnIndex).padStart(2, "0")}`;
    container.innerHTML += createColumnHTML(columnId, key);
    if (!existingColumns.includes(columnId)) existingColumns.push(columnId);
    getSnippet(key, (snippet) => updateColumn(columnId, snippet, null));
}
...
function toggleEmptyLinesVisibility() {
    document.querySelectorAll(".witness-line.om").forEach(line => line.classList.toggle("hidden"));
...
function toggleGlobalLinecounterVisibility() {
    document.querySelectorAll(".linenr-global").forEach(line => line.classList.toggle("hidden"));
...
function toggleLocalLinecounterVisibility() {
    document.querySelectorAll(".linenr_own").forEach(line => line.classList.toggle("hidden"));
...
function handleDoubleClick(event, spanId) {
    ...
    const matchingSpans = document.querySelectorAll(`.text-content span[id="${spanId}"]`);
    let highlitedSpans = []
    matchingSpans.forEach(matchingSpan => {
        if (!matchingSpan.matches(".om.hidden")) {
            ...
            if (previousVisibleSibling) {
                ...
                previousVisibleSibling.scrollIntoView({ behavior: "smooth", block: "center" });
