...
try:
    client.collections["column test (werner)"].delete()
except ObjectNotFound:
    pass

current_schema = {
    "name": "column test (werner)",
    "fields": [
        {"name": "id", "type": "string"},
...
for x in tqdm(files, total=len(files)):
    ...
    for v in facs:
        ...
        pages += 1
        cfts_record = {
            "project": "column test (werner)",
        }
        record = {}
        record["id"] = os.path.split(x)[-1].replace(".xml", f".html?tab={str(pages)}")
        cfts_record["id"] = record["id"]
        cfts_record["resolver"] = f"https://github.com/cfhaak/priester_werner/{record['id']}"
...
print(make_index)
print("done with indexing column test (werner)")

make_index = CFTS_COLLECTION.documents.import_(cfts_records, {"action": "upsert"})
print(make_index)
print("done with cfts-index column test (werner)")
